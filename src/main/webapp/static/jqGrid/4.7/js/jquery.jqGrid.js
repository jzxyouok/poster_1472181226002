	...
	parseDate : function(format, date, newformat, opts) {
		...
		if(date && date != null) {
			if( !isNaN( date - 0 ) && String(format).toLowerCase() === "u") {
				...
				timestamp = new Date( parseFloat(date) ); // dmesoft 修正日期格式化错误问题
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatter = function (rowId, cellval , colpos, rwdat, _act, irow){ // dmesoft add irow 参数
			var cm = ts.p.colModel[colpos],v;
			if(cm.formatter !== undefined) {
				rowId = String(ts.p.idPrefix) !== "" ? $.jgrid.stripPref(ts.p.idPrefix, rowId) : rowId;
				//var opts= {rowId: rowId, colModel:cm, gid:ts.p.id, pos:colpos };
				var opts= {rowId: rowId, colModel:cm, gid:ts.p.id, pos:colpos, irow:irow }; // dmesoft add irow 参数
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			var v,prp;
			v = formatter(rowId,cell,pos,srvr,'add',irow); // dmesoft add irow
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join('')); // dmesoft 恢复3.7被注释的代码，下面这一行不支持ie8
					//ts.firstElementChild.innerHTML += rowData.join(''); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
					ts.grid.cols = ts.rows[0].cells; // update cached first row
				}
			}
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("width",(nwidth+2)+"px"); // dmesoft 修正宽度计算错误
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				...
				$('#'+$.jgrid.jqID($t.p.id)+"_frozen").parent().height(bDiv.height() - 16+($.jgrid.msie?0:4)); // dmesoft 修正IE下高度计算错误
			}
			$t.p.height = nh;
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			case "select" :
				...
				$(elem).css({width:"98%"}); // dmesoft select attr add width
				var msl, ovm = [];
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height2:'+hth+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>'); // dmesoft 修正标题栏高度计算错误
				...
				if($t.p.footerrow) {
					...
					$t.grid.fsDiv = $('<div style="position:absolute;left:0px;top2:'+(parseInt(top,10)+parseInt(hth,10) + parseInt(hbd,10)+1)+'px;" class="frozen-sdiv ui-jqgrid-sdiv"></div>'); // dmesoft 修正锁定列，统计行位置计算错误
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					$($t.grid.fhDiv).height($($t.grid.hDiv).height()); // dmesoft add 修正锁定列，标题栏高度计算错误
					//$($t.grid.fbDiv).height($($t.grid.bDiv).height()-16);
					$($t.grid.fbDiv).height($($t.grid.bDiv).height()-16+($.jgrid.msie?0:4)); // dmesoft 修正IE下高度计算错误
					$($t.grid.fsDiv).css('top', $($t.grid.hDiv).height() + $($t.grid.fbDiv).height() - $($t.grid.fsDiv).height() - 1); // dmesoft 修正锁定列，统计行位置计算错误
					// dmesoft 凑合用IE8下冻结列显示问题
...
$.jgrid.extend({
	...
	expandNode : function(rc) {
		return this.each(function(){
			...
			// dmesoft add eveal onExpandNode
	...
	collapseNode : function(rc) {
		return this.each(function(){
			...
			// dmesoft add eveal onCollapseNode
