...
<script type="text/javascript">
    $("#${id}Button, #${id}Name").click(function () {
        ...
        <%--eModal.iframe(--%>
                <%--{--%>
                    <%--url: "${ctx}/tag/treeselect?url=" + encodeURIComponent("${url}") + "&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}",--%>
                    <%--title: "选择${title}",--%>
                    <%--size: 'sm'--%>
                <%--});--%>
        <%--if (1 != 1) {--%>
            // 正常打开
            $.jBox.open("iframe:${ctx}/tag/treeselect?url=" + encodeURIComponent("${url}") + "&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}",
                    "选择${title}", 300, 420, {
